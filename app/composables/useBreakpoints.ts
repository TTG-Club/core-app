 ...
 * XXL: 1536px,
 *
 * `XS` — полноценная ступень шкалы, а не служебное значение под один компонент.
 * Разрыв между 480 и 640 на мобильных большой: при 480 уже не помещаются
 * горизонтальные группы кнопок, которые при 640 стоят свободно. Шкала заведена
 * целиком в трёх местах — `@theme` в `tailwind.css`, `$xs` в `breakpoints.scss`
 * и здесь, — потому что расхождение между источниками ловится потом дороже, чем
 * стоит неиспользуемая пока константа.
 */
