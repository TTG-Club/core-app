<script setup lang="ts">
   ...
   *
   * Черновика в отборе нет: сохранить игру без публикации в разделе негде, и
   * отбор по нему всегда пуст. Подпись самого состояния остаётся — карточка
   * старой записи должна подписаться правильно.
   */
  const statusItems = computed(() => [
    { label: MY_GAMES_STATUS_ALL_LABEL, value: MY_GAMES_STATUS_ALL_VALUE },
    ...GAME_STATUSES.filter((value) => value !== 'DRAFT').map((value) => ({
