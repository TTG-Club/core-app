<script setup lang="ts">
  ...
  /**
   * Поля блока заклинаний: чем существо колдует и какими порциями.
   *
  ...
  /** Заводит порцию в конце списка. */
  function addGroup(): void {
    model.value.groups.push(createEmptyCreatureSpellGroup());
  }

  /**
   * Убирает порцию вместе с её заклинаниями.
   *
   * @param index - позиция порции в списке
   */
  ...
  /**
   * Заменяет порцию целиком.
   *
   * Порция правится по полям и приезжает тем же объектом, но присваивание
   * оставлено: без него у строки не было бы обработчика обновления, и её
   * `defineModel` ушёл бы в локальный режим.
   *
   * @param index - позиция порции в списке
   * @param group - новая порция
   */
