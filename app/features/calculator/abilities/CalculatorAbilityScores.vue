<script setup lang="ts">
   ...
   * результат уходит наверх событием `update:scores`, поэтому им пользуется и
   * страница калькулятора, и лист персонажа.
   */
  ...
  // Связь односторонняя: наборы — единственный источник значений, подставить
  // их снаружи нельзя, поэтому здесь событие, а не модель.
  const emit = defineEmits<{
    'update:scores': [scores: AbilityScores];
  }>();
  ...
  watch(activeBaseScores, (baseScores) => emit('update:scores', baseScores), {
    immediate: true,
  });
</script>
