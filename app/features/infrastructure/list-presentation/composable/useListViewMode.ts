 ...
 *
 * Настройка общая для всех разделов, поэтому хранится под одним ключом без привязки
 * к разделу — в отличие от группировки и сортировки, которые изолированы по `sectionKey`.
 * Количество колонок отдаётся готовым числом, чтобы страницы не дублировали сопоставление
 * режима с сеткой.
 *
 * До монтирования режим всегда считается дефолтным: на сервере `localStorage` недоступен,
 * и разметка собирается с сеткой. Если бы клиент читал сохранённый режим сразу, значение
 * совпало бы с серверным при первом чтении и Vue не перерисовал бы список — сохранённый
 * выбор просто игнорировался бы до следующего изменения.
 */
