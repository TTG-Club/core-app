<script setup lang="ts">
   ...
   * снимок увеличивается, а рамка уезжает в прокрутку.
   */
  ...
  // Увеличенный снимок: по щелчку вместо вписанного в экран.
  const isZoomed = ref(false);

  /**
   * Рамка: увеличенный снимок не влезает, поэтому она растёт до высоты экрана и
   * прокручивается сама. Высота та же, что у вписанного снимка с отбивками
   * рамки, — иначе прокрутка появилась бы ещё и у всего окна.
   */
  const frameClass = computed(() =>
    isZoomed.value
      ? 'max-h-[calc(100dvh-13rem)] overflow-auto rounded-lg bg-muted p-2'
      : 'flex items-center justify-center overflow-hidden rounded-lg bg-muted p-2',
  );

  /**
   * Картинка: вписанная в экран не растягивается сверх своего разрешения (от
   * растяжения снимок стал бы мыльным). Увеличенная растягивается на всю
   * ширину окна, а снимок шире окна показывается в своём настоящем размере:
   * присланный снимок часто и так влезает в экран, и без растяжения увеличение
   * ничего бы не меняло. Из высоты вычтены шапка окна и отбивки, иначе окно
   * уезжает в прокрутку целиком.
   */
  const imageClass = computed(() =>
    isZoomed.value
      ? 'min-w-full max-w-none cursor-zoom-out'
      : 'max-h-[calc(100dvh-14rem)] max-w-full cursor-zoom-in object-contain',
  );
