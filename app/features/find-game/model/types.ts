...
export interface Game {
  ...
  /**
   * Виртуальный стол. Как и чат игры, приходит только мастеру и принятым
   * игрокам — остальным сервис его не отдаёт.
   */
  virtualTableUrl: string | null;

  /**
   * Мастер указал виртуальный стол. Приходит всем, даже когда самой ссылки
   * в ответе нет: игрок заранее знает, что стол откроется после принятия.
   */
...
/**
 * Применённое условие каталога в ряду под панелью: подпись и то, как это
 * условие снять, не открывая фильтры.
 */
export interface GameFilterChip {
  key: string;
  label: string;
  /** Условие исключает значение — чип красится как исключение. */
  isExcluded: boolean;
  /** Фильтр без этого условия. */
  remove: (filter: GameSearchFilter) => GameSearchFilter;
}
