 ...
 * Строка идёт поверх обложки карточки, где на два значка нет места, а знать
 * оба условия нужно до открытия объявления.
 * @param game Игра.
 */
export function getGameFormatSummary(game: Game): string {
  const duration = GAME_DURATION_TYPE_LABELS[game.durationType];

  return `${getGameFormatLabel(game)} · ${duration}`;
}

/**
 * Счётчик найденных игр над выдачей каталога: он же показывает, что отбор
 * вообще сработал.
 * @param total Сколько игр нашлось.
 */
...
/**
 * Занятость числом. У большого состава, где значки игроков уже не читаются и
 * не рисуются, счётчик остаётся единственным: ряд мест не должен оставаться
 * пустым, иначе непонятно, набрана группа или нет.
 *
 * @param game Игра.
 */
...
/**
 * Проверяет, что начало сессии строго позже текущего момента.
 * @param startsAt Начало встречи в ISO.
 * @param currentTime Текущий момент в миллисекундах.
 */
