<script setup lang="ts">
  ...
  /**
   * Есть ли в умениях выбор черты — боевой стиль, черта за повышение
   * характеристик, черта взятого варианта умения. Выбор варианта приходит уже
   * после загрузки класса («Уроки первородных» колдуна дают черту
   * происхождения), поэтому каталог черт догружается по его появлению, а не
   * один раз на загрузке класса.
   */
  const hasFeatChoice = computed(() =>
    featureRows.value.some((row) => row.featChoices.length > 0),
  );

  // Запись класса в источнике — чтобы повторное «Далее» после сбоя каталога
  // запросило его снова: запись приходит заново, а примета остаётся прежней.
  // Цикла нет: обработчик правит только каталог черт, а ни примета, ни запись
  // класса от загруженного каталога не меняются.
  watch([hasFeatChoice, classDetail], handleFeatChoicePresenceChange);

  /**
   * Догружает каталог черт, когда в умениях появился выбор черты. Без выбора
   * черты каталог не запрашивается — иначе лишний запрос на каждое открытие
   * мастера, а пришедший или загружаемый не запрашивается повторно.
   *
   * @param watchedValues есть ли в умениях выбор черты и запись класса; запись
   *   нужна только источнику вотчера.
   */
  function handleFeatChoicePresenceChange([hasChoice]: [
    boolean,
    ClassSummary | null,
  ]): void {
