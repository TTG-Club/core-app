    ...
    return getSpellChoices(source).flatMap((choice) => {
      ...
      return (
        getPool(choice)
          ...
          // Выбранное заклинание персонаж знает, но готовит сам: пометку
          // ставит игрок, и она занимает место среди подготовленных наравне с
          // книгой, а заговор — место в числе заговоров класса. Выбор с
          // отметкой «Подготавливать не нужно» держит заклинание готовым сам и
          // места не занимает: так приходит заговор «Чудотворца» жреца.
          .map((spell) => ({
            ...spell,
            prepared: choice.alwaysPrepared === true,
            alwaysPrepared: choice.alwaysPrepared === true,
          }))
      );
    });
  }
