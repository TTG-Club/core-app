...
/** Изменение скоростей особенностями листа, приведённое к его единицам измерения. */
export interface FeatureSpeedModifiers {
...
export interface CharacterFeature {
  ...
  /**
   * Постоянные модификаторы листа от черты, вида или происхождения; null —
   * запись лист не двигает. Поля нет у особенностей, добавленных до появления
   * механики, у умений класса и у заведённых вручную.
   */
...
export interface SpeciesFeatureSummary {
  ...
  /**
   * Уровень персонажа, с которого умение действует; null — с первого либо
   * справочник его не задал.
   */
  level: number | null;

  /**
   * Постоянные модификаторы листа из `mechanics.modifiers`; null — умение лист
   * не двигает либо механика у записи справочника не заполнена.
   */
  modifiers: CharacterFeatureModifiers | null;

  /**
   * Навыки, которыми умение наделяет без выбора, названиями листа. Пусто —
   * умение владений не выдаёт.
   */
  skills: string[];

  /**
   * Выбор владения навыком, заданный у умения структурно: сколько навыков и из
...
export interface SpeciesSummary {
  ...
  /**
   * Дальность тёмного зрения в футах из свойств вида; null — справочник её не
   * задал, и дальность ищется в тексте умений.
   */
  darkVision: number | null;

  /** Описание записи в разметке сайта: текст записи-носителя своей механики. */
  description: FeatureDescriptionNode[];
  ...
  /**
   * Постоянные модификаторы от механики самой записи — того, что даёт выбор
   * вида или происхождения целиком; null — запись лист не двигает.
   *
   * Своей механикой пользуются прежде всего происхождения: умений у них нет,
   * и приписать сопротивление инфернального тифлинга больше некуда.
   */
  modifiers: CharacterFeatureModifiers | null;

  /** Навыки, которые запись выдаёт без выбора, названиями листа. */
  skills: string[];

  /**
   * Выбор владения навыком у самой записи; null — записи он не задан. Пустой
   * пул означает выбор из всех навыков.
   */
  skillChoice: ClassFeatureSkillChoice | null;
}
