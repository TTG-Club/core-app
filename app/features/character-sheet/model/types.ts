...
export interface CharacterClass {
   ...
   * «Заговоры») — сколько заговоров персонаж может знать; пусто — колонки у
   * класса нет либо лист сохранён до появления поля (тогда число задаётся
   * вручную).
   */
...
export interface CharacterSpell {
   ...
   * добавленных до этого поля. У заговоров поле не читается: подготовки они не
   * требуют, а плитка считает известные (см. `PreparedSpellKind`).
...
export interface SpellTabFilter {
  /**
   * Показывать только подготовленные заклинания. Заговоры остаются в списке
   * всегда: подготовки они не требуют.
   */
...
export interface CharacterSpellcasting {
  ...
  /**
   * Настройка числа заговоров, которые персонаж может ЗНАТЬ: подготовка их не
   * касается (см. {@link PreparedSpellKind}). Поле осталось с прежним именем —
   * оно записано в сохранённых листах.
   */
  preparedCantrips: CharacterPreparedSpells;
}

/**
 * Вид счёта заклинаний в шапке вкладки. Заклинания кругов 1+ персонаж
 * подготавливает («Подготовленные заклинания 1+ уровня» правил 2024), а
 * заговоры подготовки не требуют — они всегда доступны, и колонка «Заговоры»
 * таблицы класса говорит, сколько их персонаж знает.
 */
...
export interface PreparedKindLabels {
  ...
  /** Подпись итога в модалке настройки: «Всего можно подготовить»/«знать». */
  total: string;
...
export interface PreparedSpellsBreakdown {
  /**
   * Итоговое число: сколько заклинаний можно держать подготовленными либо
   * сколько заговоров можно знать; null — не определено (класс его не считает,
   * а своё значение не задано).
   */
  value: number | null;

  /**
   * Сколько сейчас набрано: у заклинаний — отмечено подготовленными, у
   * заговоров — известно персонажу (см. {@link PreparedSpellKind}).
   */
  count: number;
...
export interface SpellcastingBreakdown {
  ...
  /** Число известных заговоров с разбором его источников. */
