 ...
 * характеристики, суммарный уровень персонажа или уровень в своём классе.
 *
 * `class-level` отличается от `level` только у мультикласса: очки чародейства
 * равны уровню В ЧАРОДЕЕ, а не сумме уровней всех классов. У ресурса без
 * класса-владельца (свой ресурс игрока, ресурс черты или вида) он читается как
 * `level` — брать уровень не в чём.
 */
export type ResourceMaxSource =
  | 'fixed'
  | 'proficiency'
  | 'ability'
  | 'level'
...
export interface CharacterClassResource {
  ...
  /**
   * Ключ ресурса, по которому его тратит эффект («Тратит ресурс» — `rage` у
   * «Ярости»). У ресурса справочника он книжный, у своего ресурса игрока его
   * вписывают в форме; нет — эффекты на ресурс не ссылаются.
   */
  key?: string;
...
export interface CharacterClass {
   ...
   * «Заговоры») — сколько заговоров можно держать подготовленными; пусто —
   * колонки у класса нет либо лист сохранён до появления поля (тогда число
   * задаётся вручную).
   */
...
/**
 * Как игрок взял выдачу «весь список класса»: целиком (`all`) — все заклинания
 * списка ложатся на лист, и новые круги открываются сами; выбором (`chosen`) —
 * на лист ложатся только выбранные. Подготовка в обоих случаях за игроком.
 */
...
export interface CharacterFeature {
  ...
  /**
   * Как игрок взял выдачу «весь список класса» умения. `chosen` — на записи
   * лежат только выбранные им заклинания, и мастер повышения предлагает
   * добрать новые. Нет поля — список взят целиком: так вели себя листы до
   * появления выбора, и так ведёт себя умение без такой выдачи.
   */
  classSpellListMode?: ClassSpellListMode;
 ...
 * `@prof`, `@level`, `@classLevel` или `@mod.<аббревиатура>`. Разбирает её
 * `parseResourceMaxFormula` — на листе формула живёт уже правилом
 * {@link ResourceMaxRule}.
 */
...
export interface CharacterSpell {
   ...
   * добавленных до этого поля. Заговоры книги подготавливаются так же, но
   * считаются своим счётчиком (см. `PreparedSpellKind`); заговор без пометки
   * получает её при загрузке (см. `settleBookCantripsPrepared`).
  ...
  /**
   * Выдавшая запись держит заклинание подготовленным: пометка стоит сразу, а
   * места среди подготовленных заклинание не занимает — счётчик его не видит
   * (заклинания домена, врождённые заклинания вида). Заговор с отметкой так же
   * не входит в число заговоров таблицы класса («Чудотворец» жреца).
   *
   * `false` — выдача подготовку не держит: заклинание персонаж знает, но
   * готовит сам, и пометка занимает место наравне с книгой. Так приходит «весь
   * список класса» друида и жреца.
   *
   * Нет поля — запись легла на лист до появления флага: читается как «держит
   * подготовленным», ровно как лист вёл себя раньше
  ...
  /**
   * Круг заклинания ограничен ячейками персонажа: запись пришла из группы «весь
   * список класса» с отметкой «не выше доступного круга».
   *
   * Не число, а признак: граница растёт вместе с персонажем, и снимок числом
   * замёрз бы на уровне, где заклинание легло на лист. Список приезжает целиком —
   * круги отбираются каждый раз заново, там же, где проверяется
   * {@link CharacterSpell.requiredLevel} (см. `collectGrantedSpells`).
   */
  limitedBySlots?: boolean;

  /**
   * Заклинание пришло с умением правилом «весь список класса», а не
   * перечислением: по этой пометке мастер класса предлагает игроку взять список
   * целиком либо выбрать из него самому
   * ({@link CharacterFeature.classSpellListMode}). Выбранное из списка на
   * запись ложится уже без пометки: его игрок взял сам, и при пересборке умения
   * оно не уходит вместе с остальным списком.
   */
...
export interface SpellTabFilter {
  /**
   * Показывать только подготовленные заклинания и заговоры: выданные заговоры
   * подготовлены всегда.
   */
...
export interface CharacterSpellcasting {
  ...
  /**
   * Настройка числа подготовленных заговоров (свой счётчик, см.
   * {@link PreparedSpellKind}).
   */
  preparedCantrips: CharacterPreparedSpells;
}

/**
 * Вид счёта заклинаний в шапке вкладки. Заговоры книги игрок подготавливает
 * так же, как заклинания: держит их в книге про запас и отмечает те, что знает
 * сейчас. Число задаёт своя колонка таблицы класса («Заговоры»), поэтому
 * счётчики идут порознь. Выданные заговоры (вид, черта, умение класса)
 * подготовлены всегда.
 */
...
export interface PreparedKindLabels {
  ...
  /** Начало подсказки плитки: сколько уже набрано. */
  countHint: string;

  /** Подпись итога в модалке настройки. */
  total: string;
...
export interface PreparedSpellsBreakdown {
  /**
   * Итоговое число: сколько заклинаний либо заговоров можно держать
   * подготовленными; null — не определено (класс его не считает,
   * а своё значение не задано).
   */
  value: number | null;

  /**
   * Сколько сейчас отмечено подготовленными (см. {@link PreparedSpellKind}).
   */
  count: number;
...
export interface SpeciesSummary {
  ...
  /**
   * Размеры записи структурой, подписями листа: их может быть несколько — у
   * видов D&D 2024 размер бывает на выбор. Пусто — справочник структуру не
   * отдал, и мастер разбирает строку размера.
   */
  sizeOptions: string[];

  /**
   * Скорости записи числами. `null` — справочник структуру не отдал, и мастер
   * разбирает строку скорости.
   */
  speed: CharacterSpeed | null;
...
export interface ClassFeatureSummary {
  ...
  /**
   * Legacy-выбор владения навыками у самого умения: сколько навыков и из какого
   * пула. `null` — справочник его не задал, и умение о навыках не спрашивает.
   * Пустой пул означает выбор из всех навыков.
   */
...
export interface ClassSummary {
  ...
  /**
   * Приписка к владению свободным текстом («воинское оружие со свойством
   * Фехтовальное или Лёгкое»): категориями словаря выразимо не всё, и в
   * справочнике она хранится текстом. Пусто — приписки нет.
   */
  proficiencyCustom: {
    armor: string;
    weapon: string;
  ...
  /**
   * Владения, заявленные справочником: поля владений класса (категории брони и
   * оружия, спасброски) и дары механики самой записи. `null` — не заявлено ни
   * там, ни там, и мастер разбирает прозу владений.
   */
  proficiencies: GrantedProficiencies | null;

  /**
   * Выбор владения навыками, заявленный справочником: сколько навыков и из
   * какого пула. `null` — справочник его не задал.
   */
  skillChoice: ClassChoice | null;
...
export interface ClassChoice {
  ...
  /**
   * Выбранные заклинания держатся подготовленными и места в числе класса не
   * занимают: заговор «Чудотворца» жреца идёт сверх колонки «Заговоры». Без
   * отметки выбранное готовят наравне с книгой. Только для `kind: 'spell'`.
   */
  alwaysPrepared?: boolean;
...
export interface SheetChoiceControl {
  ...
  /**
   * Предупреждение под полем о повторе — своё у каждого вида выбора: навык,
   * которым персонаж уже владеет, и заклинание, которое он уже знает, ничего
   * не добавляют. Пусто — повторять в этом выборе нечего.
   */
  warning: string;
}
...
export interface LevelUpStepView {
  ...
  /**
   * Добор заклинаний «весь список класса» у умений, где игрок выбирает их сам;
   * пусто — на этом уровне добирать нечего.
   */
  classSpellListPicks: LevelUpClassSpellListPick[];
}

/**
 * Добор заклинаний списка класса на шаге повышения уровня: умение взято в
 * режиме выбора, и новый круг либо рост числа подготовленных дают повод
 * добавить заклинания.
 */
export interface LevelUpClassSpellListPick {
  /** Ключ ответа в черновике шага: у каждого уровня свой. */
  id: string;

  /** Идентификатор умения на листе — на его запись ложится выбранное. */
  featureId: string;

  featureName: string;

  /** Заклинания списка, ещё не взятые персонажем. */
  pool: CharacterSpell[];

  /** Те же заклинания вариантами пикера. */
  options: SheetChoiceOption[];
...
/** Уровень класса, на котором мастер повышения предлагает добор списка. */
export interface ClassSpellListPickContext {
  base: ClassSummary;
  subclass: ClassSummary | null;

  /** URL класса — по нему собраны идентификаторы умений на листе. */
  classUrl: string;

  /** Уровень В КЛАССЕ, который берут на этом шаге. */
  classLevel: number;

  casterType: CasterType | null;

  /** Прогрессия подготовленных заклинаний класса и подкласса. */
  preparedSpells: PreparedSpellsScaling[];

  /** Записи умений на листе: по ним видно, где игрок выбирает список сам. */
  features: CharacterFeature[];

  /** Названия заклинаний, выбранных на прошлых шагах мастера. */
