 ...
 *
 * Отдельной картой, а не запросом словаря: разбор детали черты синхронный. С
 * языками так не вышло — там справочники разошлись и в названиях, и в
 * группировке, поэтому языков в выдаче нет вовсе.
 */
...
export const SHEET_FEAT_MODAL_LABELS = {
  choiceHint:
    'Черта даёт компетентность: бонус мастерства в выбранном навыке удваивается. Выбрать можно только навык, которым персонаж уже владеет.',
  choicePlaceholder: 'Выберите навык',
  choiceEmptyOptions:
    'Персонаж пока не владеет ни одним навыком — компетентность дать не в чем. Выберите класс или предысторию, затем добавьте черту.',
