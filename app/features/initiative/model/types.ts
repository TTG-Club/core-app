...
/**
 * Боец в карусели ходов.
 *
 * Карусель рисует токен и подпись — больше ей ничего не нужно. Отдельный тип
 * нужен потому, что показывают её не только по трекеру: игровая комната
 * получает бой снимком, и участника трекера у неё на руках нет.
 */
export interface ReelParticipant {
  ...
  /** Повержен: токен гаснет, но места в очереди не теряет. */
  dead: boolean;

  /** Результат броска инициативы; не задано — ещё не бросали. */
  initiativeTotal?: number;

  /** Цвет токена; не задано — цвет по умолчанию. */
  color?: ParticipantColor;

  /** Картинка токена; не задано — иконка по типу. */
  avatarUrl?: string;
}
