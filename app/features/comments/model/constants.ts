...
/** Подпись бейджа платформы-источника в модерации. */
export const COMMENT_SOURCE_PLATFORM_LABEL = 'Платформа';

/**
 * Подсказка на погашенной кнопке «Открыть», когда комментарий пришёл с другой
 * платформы, а её адрес не задан в конфиге.
 */
export const ADMIN_COMMENT_FOREIGN_SITE_HINT =
  'Комментарий с другого сайта — адрес не задан в NUXT_PUBLIC_OLD_SITE_URL';
...
/**
 * Значение фильтра платформы «все» в модерации. Не пустая строка: выбранный пункт
 * селекта должен иметь конкретное значение, а «все» на бэкенд уходит отсутствием
 * параметра (см. страницу модерации).
 */
export const ADMIN_COMMENTS_PLATFORM_ALL = 'all';

/** Подпись пункта «все платформы» в фильтре модерации. */
export const ADMIN_COMMENTS_PLATFORM_ALL_LABEL = 'Все платформы';
...
/**
 * Названия разделов сайта для карточки: где именно оставлен комментарий.
...
export const MY_COMMENTS_UNKNOWN_SECTION_LABEL = 'Неизвестный раздел';
