...
/**
 * Пауза антиспам-лимита между отправками комментариев. Лимит сервиса
 * действует на пользователя целиком, поэтому состояние одно на всё
 * приложение (`useState`) — отправка в дровере ставит на паузу и форму на
 * странице. Модераторов и администраторов сервис не ограничивает — для них
 * пауза не включается.
 */
export function useCommentSubmitCooldown(): UseCommentSubmitCooldownReturn {
  const cooldownUntil = useState<number>(
    'comments-submit-cooldown-until',
    () => 0,
  );
  ...
  function startCooldown(seconds: number): void {
    if (canModerateComments.value || seconds <= 0) {
      return;
    }
