...
/** Раздел S3 для картинок к постам — первый сегмент ключа объекта. */
export const PUBLICATION_IMAGE_SECTION = 'game-publications';
/**
 * Максимальная длина короткой стороны картинки. Telegram и ВКонтакте
...
/**
 * Адрес картинки из загрузки сайта: путь `/s3/...` или он же с адресом сайта.
 * Сайт передаёт полный адрес, потому что у стендов разные хранилища картинок.
 */
export const PUBLICATION_IMAGE_PATTERN =
  /^(?:https:\/\/[a-z\d.-]{1,60})?\/s3\/(?:[\w-]+\/){1,4}[\w-]+(?:\.[a-z\d]{1,10})?$/i;
