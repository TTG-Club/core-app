...
export interface FeatGrantedSpellRef extends FeatEntityRef {
  ...
  /**
   * Характеристика, от которой считаются ЭТИ заклинания. `undefined` — группа её не
   * задаёт, и характеристику берут выше: сперва ответ игрока, затем характеристика
   * записи, затем класс, чья это магия.
   *
   * У группы, а не только у записи целиком: один набор заклинаний записи может
   * считаться от одной характеристики, другой — от другой, и одно поле на всех
   * заставило бы заводить ради этого вторую запись.
   */
  spellcastingAbility?: AbilityKey;

  /**
   * ЭТИ заклинания не нужно готовить. `undefined` — группа отметку не задаёт, и её
   * берут у записи.
   *
   * По той же причине, что и характеристику: заклинания домена всегда подготовлены,
   * а заклинание, выданное тем же умением сверх них, подготовку занимает.
   */
 ...
 *
 * Перечислением ({@link FeatSpellGrant.spells}) такое не описывается: список пришлось
 * бы дописывать руками при каждом пополнении справочника, и персонаж, созданный
 * вчера, отличался бы от созданного сегодня не по правилам, а по тому, когда автор
 * дошёл до записи. Здесь хранится правило — заклинания подставляет core-api при
 * отдаче детали, поэтому новое заклинание класса достаётся новым персонажам само.
 *
 * Групп бывает несколько, и это НЕ взаимоисключающие варианты: каждая открывается на
 * своём уровне и складывается с предыдущими.
 */
export interface FeatClassSpellListGrant {
  /**
   * Уровень персонажа, с которого группа выдаётся; `undefined` — с момента взятия
   * записи.
   */
  requiredLevel?: number;

  /**
   * Классы, чьи списки выдаются. Несколько — объединение списков, а не выбор: выбор
   * списка описывается выбором игрока (`SPELL_LIST`), а здесь выдача без выбора.
   */
  classes: Array<FeatEntityRef>;

  /** Ровно этот круг: «все заговоры друида»; `undefined` — круг сверху не задан. */
  level?: number;

  /** Не выше этого круга; `undefined` — верхней границы нет. */
  maxLevel?: number;
   ...
   *
   * Сервер такую границу поставить не может — персонажа он не знает, — поэтому
   * заклинания приезжают целиком с пометкой `limitedBySlots`, а круг режет лист.
   * Взведён — `level` и `maxLevel` не заполняются: три границы разом означали бы три
   * разных ответа на один вопрос.
   */
  maxLevelFromSlots?: boolean;
...
export interface FeatSpellGrant {
   ...
   *
   * Рядом с {@link FeatSpellGrant.spells}, а не вместо них: перечень называет
   * заклинания поимённо, а группа описывает правило и разворачивается при выдаче — так
   * новое заклинание класса достаётся новым персонажам без правки записи. Одна запись
   * может и перечислять, и выдавать список: у каждой группы свой уровень открытия.
   */
  classLists: Array<FeatClassSpellListGrant>;
 ...
 * МОЖЕТ выучить или подготовить наравне с классовыми, потратив на него
 * подготовку и ячейку. Свалить их в одну кучу значило бы выдать «Метке
 * исцеления» всю её таблицу готовыми заклинаниями.
 *
...
export interface FeatSpellListGroup {
  ...
  /**
   * Заклинания списка целиком: расширение не спрашивает «сколько взять» — оно
   * лишь добавляет заклинания к списку класса, а учит или готовит их персонаж
   * обычным порядком. «Выбрать N из перечисленных» — это выбор заклинаний
   * ({@link FeatSpellPickRow} с перечисленными заклинаниями), а не расширение.
   * Круг берётся из самой записи справочника.
   */
...
export interface FeatCounter {
  ...
  shortName: string;
  /**
   * Формула максимума: число, `@prof`, `@level`, `@classLevel`,
   * `@mod.<abbr>`. `@classLevel` — уровень в классе, выдавшем запись; там, где
   * класса-владельца нет (черта, вид, предмет), он читается как `@level`.
   */
  max: string;
   ...
   * (ступени, число, `@prof`, `@level`, `@classLevel`); у максимума по модификатору
