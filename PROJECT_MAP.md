...
| `infrastructure` | Cross-cutting app shell & chrome                                            | `sidebar`, `search`, `filter`, `list-presentation`, `footer` (legal links to `/privacy` and `/cookies`), `cookie-consent` (consent banner + the `/cookies` page text in `model/policy.ts`: in plain language why the site keeps anything in the browser, no cookie names), `privacy-policy` (`model` only — the 152-ФЗ policy text for `/privacy`, no payments section), `pwa` |
...
| `legal-document`  | Legal document page body: intro, edition-date badge, sections with optional icon/badge and one level of subsections; paragraphs, bullets and links (`to` for site pages, `href` for `mailto:`/external). Renders `/cookies` and `/privacy`                                                                                                                                       |
