...
| `home`           | Landing-page building blocks composed on `pages/index.vue` (layout `home`): a full-bleed hero, the section plate, then the feed. From `xl` the feed splits into a 3/4 part of two rows and a separate 1/4 column: each row is `items-stretch`, so news set the height of the first and articles of the second, while their row neighbours follow — heights can only be tied between siblings of one row. The right column knows nothing of those rows, so it flows straight through. All panels share the `ui-kit` chrome (frosted glass over the particle canvas) and monospaced eyebrow labels | `hero` (headline, search, live counters and tool chips), `ui-kit` (`HomePanel` with `fill`/`header`/`body` slots, `HomeEyebrow`), `news` (hero cover + tile grid), `articles` (separate index block from `news`), `activity` (comments and catalog updates as two halves of one split heading, comments first), `sections` (one plate of hairline-divided tiles: an icon names the section, the photo is desaturated texture), `banners` (VTTG promo), `tools` (chip row inside the hero, role-gated items), `community` (tagline + bug-hunter rating), `counters` (`HomeCounters` strip in the hero), `party-finder` (reserved slot for the future game-search service — a dashed wireframe of the card to come), `greetings`, `recent-changes`, `background`, `social-links`, `link-to-5e14` |
