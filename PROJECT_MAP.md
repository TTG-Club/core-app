...
| `infrastructure` | Cross-cutting app shell & chrome                                            | `sidebar`, `search`, `filter`, `list-presentation`, `footer`, `cookie-consent` (consent banner + `policy` — the `/cookies` page telling in plain language why the site keeps anything in the browser: no cookie names and no per-cookie table, one section per reason; linked from the banner and the footer), `pwa` |
...
## 🎨 Shared UI Kit (`app/shared/ui/` — 33 components)
